...
console.log(`pattern.test: ${pattern.test(text)}`); // true, test that a match exists

/* string methods */
console.log(`text.search: ${text.search(pattern)}`); // 9: position of first match
console.log(`text.match: ${text.match(pattern)}`); // array of matches
console.log(`text.replace: ${text.replace(pattern, "#")}`); // replace all matches with #
console.log(`"1  , 2,3".split: ${"1  , 2,3".split(/\s*,\s*/)}`); // split with comma surrounded by arbitrary whitespace
